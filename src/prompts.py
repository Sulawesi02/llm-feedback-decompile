...
3. 只输出 C 代码，不要解释。
"""},
        {"role": "assistant", "content": f"""请根据以下正确的 C 代码，生成一份有缺陷的代码：
{c_code}
"""}
    ]
    return messages
...
def construct_equal_prompt(c_code_1: str, c_code_2: str) -> List[Dict[str, str]]:
    ...
    """
    messages = [
        {"role": "user", "content": """你是一名资深的 C 语言代码审计专家。
...
4. 严禁输出任何解释、分析或其他文字。
"""},
        {"role": "assistant", "content": f"""请判断以下两个 C 函数是否在语义上等价。
...
def construct_infer_prompt(arch: str, asm: str) -> List[Dict[str, str]]:
    ...
    """
    messages = [
        {"role": "user", "content": """你是一名专业的 C 语言反编译专家，精通 x86-64 和 AArch64 汇编。
...
"""},
        {"role": "assistant", "content": f"""请将以下汇编代码反编译为在语义上等价的 C 函数代码。
架构：{arch}
汇编代码:
{asm}
"""}
...
def construct_fix_prompt(arch: str, asm: str, previous_c_code: str = None, error_message: str = None) -> List[Dict[str, str]]:
    ...
    """
    messages = [
        {"role": "user", "content": """你是一名专业的 C 语言反编译专家。
...
5. 严禁输出任何 Markdown（如 ```c）、多余空行或说明。
"""},
        {"role": "assistant", "content": f"""上次生成的 C 函数代码有错误，请修复。
架构：{arch}
汇编代码:
{asm}
上次生成的 C 函数代码：
